...
namespace Steins
{
	...
	namespace Utils
	{
		...
		static void AttachColorTexture(u32 id, int samples, GLenum internalFormat, GLenum format, u32 width, u32 height, int index)
		{
		...
		static void AttachDepthTexture(u32 id, int samples, GLenum format, GLenum attachmentType, 
		...
		static GLenum SteinsFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		if (m_ColorAttachmentSpecifications.size())
		{
			...
			for (u64 i = 0; i < m_ColorAttachments.size(); i++)
			{
		...
		{
			STS_CORE_ASSERT(m_ColorAttachments.size() <= 4);
			GLenum buffers[4] = {
	...
	{
		STS_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(u32 attachmentIndex, int value)
	{
		STS_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());

		auto& spec = m_ColorAttachmentSpecifications[attachmentIndex];
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0, 
			Utils::SteinsFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
