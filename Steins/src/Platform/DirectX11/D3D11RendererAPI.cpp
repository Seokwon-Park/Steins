...
namespace Steins
{
	void D3D11RendererAPI::Init(GraphicsContext* context)
	{
		...
		m_Context->GetD3DContext()->OMSetBlendState(m_Context->GetBS(), nullptr, 0xFFFFFFFF);
		m_Context->GetD3DContext()->OMSetDepthStencilState(m_Context->GetDSS(), 0);
	}
	...
	void D3D11RendererAPI::DrawIndexed(const std::shared_ptr<VertexArray>& vertexArray, u32 indexCount)
	{
		...
		m_Context->GetD3DContext()->OMSetRenderTargets(1, m_Context->GetRTVAddr(), m_Context->GetDSV());
		...
		m_Context->GetD3DContext()->CopyResource(m_Context->GetBackbuffer(), backBuffer.Get());
